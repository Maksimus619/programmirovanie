...
int main(int argc, char *argv[])
{
    ...
    struct particle *f = malloc(sizeof(*f) * n); // Ð¡Ð¸Ð»Ð°, Ð´ÐµÐ¹ÑÑÐ²ÑÑÑÐ°Ñ Ð½Ð° ÐºÐ°Ð¶Ð´ÑÑ ÑÐ°ÑÑÐ¸ÑÑ (x, y, z)
    ...
    { // Ð¦Ð¸ÐºÐ» Ð¿Ð¾ Ð²ÑÐµÐ¼ÐµÐ½Ð¸ (Ð¼Ð¾Ð´ÐµÐ»ÑÐ½Ð¾Ð¼Ñ)
        tforces -= wtime();
        calculate_forces(p, f, m, n); // ÐÑÑÐ¸ÑÐ»ÐµÐ½Ð¸Ðµ ÑÐ¸Ð» â O(N^2)
        tforces += wtime();
        tmove -= wtime();
        move_particles(p, f, v, m, n, dt); // ÐÐµÑÐµÐ¼ÐµÑÐµÐ½Ð¸Ðµ ÑÐµÐ» O(N)
        tmove += wtime();
    }
    ttotal = wtime() - ttotal;
    printf("# NBody (n=%d)\n", n);
